...
import project.dao.spec.ClientRoleDao;
...
public class ClientRoleDaoImpl implements ClientRoleDao {
	...
	private static final String INSERT = "INSERT INTO USER_ROLE (user_account_id, role) " + " VALUES (? , ?)";
	...
	public List<RoleDto> getBy(String userId) throws IOException {
		try {
			return template.query (FETCH_BY_ID, ps -> ps.setString (1, userId), (rs, rownum) -> {return new RoleDto(rs.getString("role"));});
	...
	@Override
	public void insert(String userId, List<RoleDto> roleList) throws IOException {
		try {
